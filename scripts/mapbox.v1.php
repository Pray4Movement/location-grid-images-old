...
//    }
      ...
      "features" => [
        [ "type"=> "Feature",
          "geometry"=> [
            ...
            "coordinates"=> [
                [
                    [ (float) $query_inner['west_longitude'], (float) $query_inner['north_latitude'] ],
                    [ (float) $query_inner['west_longitude'], (float) $query_inner['south_latitude'] ],
                    [ (float) $query_inner['east_longitude'], (float) $query_inner['south_latitude'] ],
                    [ (float) $query_inner['east_longitude'], (float) $query_inner['north_latitude'] ],
                    [ (float) $query_inner['west_longitude'], (float) $query_inner['north_latitude'] ]
                ]
            ]
          ],
          "properties"=> [
            ...
            'stroke-width'=> '2',
            'fill'=> '#FF0000',
            'fill-opacity'=> ($query_inner['level'] === '0') ? '0': '0.6'
          ]
        ]
      ]
    ];
    ...
    shell_exec('curl -g "https://api.mapbox.com/styles/v1/mapbox/satellite-v9/static/geojson('.$geojson.')/['.(float) $query_inner['c_west_longitude'].','.(float) $query_inner['c_south_latitude'].','.(float) $query_inner['c_east_longitude'].','.(float) $query_inner['c_north_latitude'].']/600x425@2x?padding=10,10,35&access_token=pk.eyJ1IjoiY2hyaXNjaGFzbSIsImEiOiJjajZyc2poNmEwZTdqMnFuenB0ODI5dWduIn0.6wKrDTf2exQJY-MY7Q1kRQ" --output images/locations/'.$query_inner['grid_id'].'.png');

//    $im = imagecreatefrompng('images/locations/'.$query_inner['grid_id'].'.png');
//    $im2 = imagecrop($im, ['x' => 0, 'y' => 0, 'width' => '1200', 'height' => '800']);
//    if ($im2 !== FALSE) {
//        unlink('images/locations/'.$query_inner['grid_id'].'.png');
//        imagepng($im2, 'images/locations/'.$query_inner['grid_id'].'.png');
//        print '*';
//    } else {
